...
+(function ($) {
	...
	EditableSelect.prototype.holder = function (textholder) {
		this.$input.attr('placeholder',textholder);
	};

	EditableSelect.prototype.clean = function () {
		this.$input.val('');
	};
			...
			switch (e.keyCode) {
				...
				case 13: // Enter
					if (that.es.$list.is(':visible')) {
						...
						that.es.$input.blur();
						//that.es.$input.setCaretPos(that.es.$input.val().length);
						//that.es.$input.next(':input').focus();
						//e.preventDefault();
					}
					break;
				case 9:  // Tab
