<x-app-layout>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function checkDuplicatedDPI() {
                const dpi_value = $('#dpi').val().trim();

                $.ajax({
                    url: "{{url('api/check-dpi')}}",
                    type: "POST",
                    data: {
                        dpi: dpi_value,
                        _token: '{{csrf_token()}}'
                    ...
                    success: function (result) {
                        if (result == 0) {
                            ...
                            showSimpleAlert('El DPI ingresado ya está registrado', 'Si estás seguro que no te registraste antes, envía un correo a preguntas@fiscalsemilla.com para darle seguimiento a tu caso.', 'De acuerdo','warning');
                        }
                    },
                    error: function(xhr, status, error) {
                        if (xhr.status === 404) {
                            showSimpleAlert('Problema interno','Intenta en otro momento','Entendido',null);
                        } else {
